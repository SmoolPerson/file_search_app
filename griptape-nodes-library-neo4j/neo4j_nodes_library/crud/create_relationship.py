...
class CreateRelationship(ControlNode):
    ...
    def _build_node_pattern(self, variable: str, labels: list[str], properties: dict) -> tuple[str, dict, str]:
        """Builds a node pattern for MATCH queries (properties go in WHERE clause).

        Returns:
            Tuple of (pattern_string, parameters_dict, where_conditions_string)
        """
        ...
        if properties:
            ...
            return f"({variable}{label_part})", params, " AND ".join(conditions)

        return f"({variable}{label_part})", {}, ""

    def _build_merge_node_pattern(self, variable: str, labels: list[str], properties: dict) -> tuple[str, dict]:
        """Builds a node pattern for MERGE queries (properties embedded inline).
        ...
        if properties:
            ...
            for prop, value in properties.items():
                param_name = f"{variable}_{prop.replace('.', '_')}"
                prop_parts.append(f"{prop}: ${param_name}")
                params[param_name] = value
            props_str = "{" + ", ".join(prop_parts) + "}"
            return f"({variable}{label_part} {props_str})", params
    ...
    def _build_create_query(  # noqa: PLR0913
        ...
        *,
        ...
        if create_missing:
            # Use MERGE with inline properties so Neo4j matches on the right node
            start_pattern, start_params = self._build_merge_node_pattern("start", start_labels, start_props)
            all_params.update(start_params)
            end_pattern, end_params = self._build_merge_node_pattern("end", end_labels, end_props)
            all_params.update(end_params)

            query_parts.append(f"MERGE {start_pattern}")
            query_parts.append(f"MERGE {end_pattern}")
            query_parts.append(f"MERGE (start)-[r:{rel_type}]->(end)")
            if rel_props:
                query_parts.append(f"SET r += ${rel_param_name}")
        else:
            # Use MATCH for nodes (properties go in WHERE), CREATE for relationship
            ...
            if end_where:
                query_parts.append(f"MATCH {end_pattern} WHERE {end_where}")
            else:
                query_parts.append(f"MATCH {end_pattern}")

            query_parts.append(f"CREATE (start)-[r:{rel_type}]->(end)")
            if rel_props:
                query_parts.append(f"SET r = ${rel_param_name}")
    ...
    def process(self) -> None:  # noqa: PLR0915
        ...
        try:
            ...
            error_msg = f"❌ Failed to create relationship: {e!s}, properties: {rel_props_str}, {start_props_str}, {end_props_str}"
